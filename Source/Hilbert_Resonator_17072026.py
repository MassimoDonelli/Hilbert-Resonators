# .. @ Massimo Donelli, July, 2026 ..
# .. 17/07/2026 Mistake on DXF generation corrected MD ..
...
def curve_length_freq(side_square,level,EPS):
    ...
    length = (((side_square*1e-3)/(2**level - 1))*(4**level - 1))
    # .. Estimated resonance frequency ..
    frequency = C0/((length)*pow(EPS,1/2))
...
def DXF_Thickness_Conversion(thickness):
    if(thickness<0.13 or thickness == 0.13):
        ...
        return int(200)
# .. Thess functions generate a DXF file for PCB printing ..
def movement_execution(DXF_MODEL_SPACE,L_STRING,angle,step,TRACE_THICKNESS):
#
        ...
        for char in L_STRING:
            if char == 'F':      
                ...
                if(ANGLE==0 or ANGLE==360 or ANGLE==-360):
                    ...
                    #
                    PX.append(START_X)
                    PY.append(START_Y-TRACE_THICKNESS)
                    #
                    PX.append(START_X-step)
                    PY.append(START_Y-TRACE_THICKNESS)
                    #
                    PX.append(START_X-step)
                    PY.append(START_Y)
                    ...
                    # .. Add the other side ..
                    PX.append(START_X-TRACE_THICKNESS)
                    PY.append(START_Y)
                    #
                    PX.append(START_X-TRACE_THICKNESS)
                    PY.append(START_Y+step)
                    #
                    PX.append(START_X)
                    PY.append(START_Y+step)
                    #
                ...
                ANGLE-=angle   
    # .. Now print in the DXF ..
    # .. First trace border ..
    for J_X_FOR in range(len(PX)-1):
        DXF_MODEL_SPACE.add_line((PX[J_X_FOR], PY[J_X_FOR]),(PX[J_X_FOR+1], PY[J_X_FOR+1]), dxfattribs={"lineweight":TRACE_DXF})
...
def draw_hilbert_dxf(L_STRING, step, angle, TRACE_THICKNESS, FILE_NAME):
    ...
    msp = doc.modelspace()
    movement_execution(msp,L_STRING,angle,step,TRACE_THICKNESS)    
    # .. Save file ..      
...
def main():
    ...
    draw_hilbert_dxf(HILBERT_STR,(square_side/(2**level-1)), angle, trace_thickness,file_name+'.dxf')
